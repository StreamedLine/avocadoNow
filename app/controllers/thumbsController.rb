class ThumbsController < ApplicationController
	get '/places/:id/thumbs/new' do 
		protect_data
		recent_thumb = current_user.thumbs.order('updated_at desc').select{|t| t.user_id == current_user.id}.first 
		if recent_thumb && recent_thumb.place_id == params[:id] && Time.now - recent_thumb.updated_at < 10800
